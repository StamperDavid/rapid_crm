/**
 * Client Google Login Component
 * Provides Google OAuth login for transportation compliance clients
 */
...
interface ClientGoogleLoginProps {
  onLoginSuccess: (clientAccount: any) => void;
  onLoginError: (error: string) => void;
  className?: string;
}

const ClientGoogleLogin: React.FC<ClientGoogleLoginProps> = ({ 
  onLoginSuccess, 
  onLoginError, 
  className = '' 
}) => {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      onLoginError('Failed to initiate Google login. Please try again.');
      setIsLoading(false);
    }
  };
    ...
    <div className={`space-y-4 ${className}`}>
      <div className="text-center">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
          Sign in with your Google account to access your compliance dashboard
        </p>
      </div>
