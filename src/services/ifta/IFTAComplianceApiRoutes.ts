/**
 * IFTA Compliance API Routes
 * Integrated with Rapid CRM Database
 */

import { Router } from 'express';
import { ComprehensiveIFTAService } from './ComprehensiveIFTAService';
import { Database } from 'sqlite3';

export class IFTAComplianceApiRoutes {
  private router: Router;
  private iftaService: ComprehensiveIFTAService;
  ...
  private setupRoutes() {
    ...
    this.router.get('/registrations', async (req, res) => {
      try {
        ...
        const registrations = await this.iftaService.getIFTARegistrations(companyId as string);
    ...
    this.router.get('/compliance-alerts', async (req, res) => {
      try {
        const { companyId } = req.query;
        const alerts = await this.iftaService.getComplianceAlerts(companyId as string);
        res.json(alerts);
      } catch (error) {
...
export function createIFTAComplianceApiRoutes(database: Database): IFTAComplianceApiRoutes {
